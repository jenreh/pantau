---
name: appkit-commons
description: appkit-commons usage patterns: configuration (YAML profiles, env overrides, secrets), service registry, repository pattern, database entities, custom column types (EncryptedString, ArrayType), and scheduler (APScheduler, PGQueuer). Apply when adding new features, services, repositories, scheduled tasks, or configuring the application stack.
metadata:
